...
def build_epl_label(
    *,
    ...
    if size_key == "2x1":
        ...
        title_xy_mul = (1, 2)  # Narrower font (2x1 ratio)
        text_font = 3
        text_xy_mul = (1, 1)
        narrow = 3
        wide = 6
    ...
    safe_title = truncate_text(title or "", 60)  # Increased for narrower font
    safe_item = truncate_text(item_number or "", 36)
    safe_case = truncate_text(casepack or "", 36)
    
    # Wrap title to multiple lines if needed
    title_lines = wrap_text(safe_title, 60)  # Increased for narrower font
